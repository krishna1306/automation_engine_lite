...
def take_action(
    ...
    logging.info(
        f"POST request from {request.client.host} - (3) Set Link BW Level : Successfully changed level to --> {change_value.value}"
    )
...
async def get_link_kpis(
    link_id: str, credentials: Annotated[HTTPBasicCredentials, Depends(security)]
):
...
def set_link_bw_level(
    level,
    kpi_alert_tracker: KpiAlertTracker,
    credentials: Annotated[HTTPBasicCredentials, Depends(security)],
    request: Request,
):

    logging.info(
        f"POST request from {request.client.host} - (1) Set Link BW Level : Received to change to level --> {level}"
    )

    if kpi_alert_tracker.current_state != "c":
        logging.error(
            f"POST request - Set Link BW Level : Invalid current state in payload. Current state should be c. But it is {kpi_alert_tracker.current_state}"
        )
        return {
    ...
    set_level = level
    logging.info(
        f"POST request from {request.client.host} - (2) Set Link BW Level : Internally changed level to --> {level}"
    )
    ...
    result = subprocess.run(
        ["bash", script_path, set_level], stdout=subprocess.PIPE, text=True
    )
    ...
    return {
        "status": "success",
        "data": {"payload": kpi_alert_tracker.model_dump(), "output": result.stdout},
    }


@app.get("/get_link_bw_level/{link_id}")
async def get_link_bw_level(
    link_id: str, credentials: Annotated[HTTPBasicCredentials, Depends(security)]
):
    ...
    return {"status": f"Current level is {current_level}"}
